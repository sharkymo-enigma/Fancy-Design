...
- **Cycle 1**: Sidebar gradient needs multi-stop depth (3-5 stops) with subtle radial glow overlays for premium feel. Single two-color gradients look flat. Add pointer-events-none gradient overlays for ambient light effect.
- **Cycle 2**: Chat inputs must be full-width — never constrain with max-w on the input area. Users expect full-width typing like modern chat apps (Slack, Teams). Micro-service action cards in chat make AI results actionable — include "Add to case" and "Open" buttons directly on function call results.
- **Cycle 3**: IMF logo should use SVG with proper IMF circular branding. "New conversation" button must connect to agent context ("Ask HR Robin" / "New chat") — generic labels break the mental model. Session privacy indicators (lock icon, "Private session" label) build trust in AI chat.
